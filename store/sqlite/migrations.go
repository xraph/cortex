...
`)
		...
		&migrate.Migration{
			...
			Up: func(ctx context.Context, exec migrate.Executor) error {
				...
				// from cortex_sessions right above. SQLite has neither
				// JSONB nor TIMESTAMPTZ: JSON lands in TEXT, and
				// timestamps in TEXT defaulted from datetime('now').
				if _, err := exec.Exec(ctx, `
...
`); err != nil {
			...
			Down: func(ctx context.Context, exec migrate.Executor) error {
				_, err := exec.Exec(ctx, `DROP TABLE IF EXISTS cortex_suspensions`)
				return err
			},
		},
		&migrate.Migration{
...
`); err != nil {
		...
		&migrate.Migration{
			...
			Up: func(ctx context.Context, exec migrate.Executor) error {
				...
				// tables that had to have them added later: nothing has
				// ever written an unscoped a2a row, so there is no
				// backfill to do and no scope migration to follow.
				//
				// The unique index on pending asks is load-bearing rather
				// than hygiene. ClaimPendingAsk resolves exactly one
				// waiting run, and two rows sharing a reply-with token
				// would let a reply resume a run that never asked.
				_, err := exec.Exec(ctx, `
