...
func (s *Store) CreateMessage(ctx context.Context, e *a2a.Envelope) error {
	...
	e.UpdatedAt = now
	e.Scope = scope
	if _, err := s.sdb.NewInsert(a2aMessageToModel(e)).Exec(ctx); err != nil {
		if isUniqueViolation(err) {
			return fmt.Errorf("cortex/sqlite: create a2a message: %w", cortex.ErrAlreadyExists)
		}
		return fmt.Errorf("cortex/sqlite: create a2a message: %w", err)
	}
	return nil
}
...
func (s *Store) GetMessage(ctx context.Context, msgID id.MessageID) (*a2a.Envelope, error) {
	...
	q := s.sdb.NewSelect(m).Where("id = ?", msgID.String())
	...
	if err := q.Scan(ctx); err != nil {
		...
		return nil, fmt.Errorf("cortex/sqlite: get a2a message: %w", err)
	}
...
func (s *Store) ListMessages(ctx context.Context, filter *a2a.MessageListFilter) ([]*a2a.Envelope, error) {
	...
	if err := q.Scan(ctx); err != nil {
		return nil, fmt.Errorf("cortex/sqlite: list a2a messages: %w", err)
	}
...
func (s *Store) CreateConversation(ctx context.Context, c *a2a.Conversation) error {
	...
	c.UpdatedAt = now
	c.Scope = scope
	if _, err := s.sdb.NewInsert(a2aConversationToModel(c)).Exec(ctx); err != nil {
		if isUniqueViolation(err) {
			return fmt.Errorf("cortex/sqlite: create a2a conversation: %w", cortex.ErrAlreadyExists)
		}
		return fmt.Errorf("cortex/sqlite: create a2a conversation: %w", err)
	}
	return nil
}
...
func (s *Store) GetConversation(ctx context.Context, convID id.ConversationID) (*a2a.Conversation, error) {
	...
	q := s.sdb.NewSelect(m).Where("id = ?", convID.String())
	for _, p := range scopePredicates(scope, false) {
		q = q.Where(p.Column+" = ?", p.Value)
	}
	if err := q.Scan(ctx); err != nil {
		...
		return nil, fmt.Errorf("cortex/sqlite: get a2a conversation: %w", err)
	}
...
func (s *Store) GetConversationByPeerContext(ctx context.Context, node, peerContext string) (*a2a.Conversation, error) {
	...
	if err := q.Scan(ctx); err != nil {
		...
		return nil, fmt.Errorf("cortex/sqlite: get a2a conversation by peer context: %w", err)
	}
	return a2aConversationFromModel(m)
}
...
func (s *Store) UpdateConversation(ctx context.Context, c *a2a.Conversation) error {
	...
	q := s.sdb.NewUpdate(a2aConversationToModel(c)).
	...
	res, err := q.Exec(ctx)
	if err != nil {
		return fmt.Errorf("cortex/sqlite: update a2a conversation: %w", err)
	}
	n, rowsErr := res.RowsAffected()
	if rowsErr != nil {
		return fmt.Errorf("cortex/sqlite: update a2a conversation rows affected: %w", rowsErr)
	}
	if n == 0 {
...
func (s *Store) ListConversations(ctx context.Context, filter *a2a.ConversationListFilter) ([]*a2a.Conversation, error) {
	...
	if err := q.Scan(ctx); err != nil {
		return nil, fmt.Errorf("cortex/sqlite: list a2a conversations: %w", err)
	}
...
func (s *Store) CreateDelivery(ctx context.Context, d *a2a.Delivery) error {
	...
	d.UpdatedAt = now
	d.Scope = scope
	if _, err := s.sdb.NewInsert(a2aDeliveryToModel(d)).Exec(ctx); err != nil {
		if isUniqueViolation(err) {
			return fmt.Errorf("cortex/sqlite: create a2a delivery: %w", cortex.ErrAlreadyExists)
		}
		return fmt.Errorf("cortex/sqlite: create a2a delivery: %w", err)
	}
	return nil
}
...
func (s *Store) UpdateDelivery(ctx context.Context, d *a2a.Delivery) error {
	...
	q := s.sdb.NewUpdate(a2aDeliveryToModel(d)).
	...
	res, err := q.Exec(ctx)
	if err != nil {
		return fmt.Errorf("cortex/sqlite: update a2a delivery: %w", err)
	}
	n, rowsErr := res.RowsAffected()
	if rowsErr != nil {
		return fmt.Errorf("cortex/sqlite: update a2a delivery rows affected: %w", rowsErr)
	}
	if n == 0 {
...
func (s *Store) ClaimDelivery(ctx context.Context, deliveryID id.DeliveryID) (*a2a.Delivery, error) {
	...
	now := time.Now().UTC()
	q := s.sdb.NewUpdate((*a2aDeliveryModel)(nil)).
	...
	res, err := q.Exec(ctx)
	if err != nil {
		return nil, fmt.Errorf("cortex/sqlite: claim a2a delivery: %w", err)
	}
	n, rowsErr := res.RowsAffected()
	if rowsErr != nil {
		return nil, fmt.Errorf("cortex/sqlite: claim a2a delivery rows affected: %w", rowsErr)
	}
	if n == 0 {
...
func (s *Store) ListInbox(ctx context.Context, agentName string, filter a2a.InboxFilter) ([]*a2a.Delivery, error) {
	...
	if err := q.Scan(ctx); err != nil {
		return nil, fmt.Errorf("cortex/sqlite: list a2a inbox: %w", err)
	}
...
func (s *Store) ListQueuedDeliveries(ctx context.Context, limit int) ([]*a2a.Delivery, error) {
	...
	if err := q.Scan(ctx); err != nil {
		return nil, fmt.Errorf("cortex/sqlite: list queued a2a deliveries: %w", err)
	}
	out := make([]*a2a.Delivery, len(models))
	for i := range models {
		d, convErr := a2aDeliveryFromModel(&models[i])
...
func (s *Store) ReclaimStaleDeliveries(ctx context.Context, olderThan time.Time, limit int) (int, error) {
	// Sqlite has no UPDATE ... LIMIT in the default build, so the rows
	// are selected first and updated by id. The claim predicate is on the
	// update either way, so a worker that comes back to life still wins
	// its own row.
	var models []a2aDeliveryModel
	q := s.sdb.NewSelect(&models).
	...
	if err := q.Scan(ctx); err != nil {
		return 0, fmt.Errorf("cortex/sqlite: list stale a2a deliveries: %w", err)
	}

	var n int
	now := time.Now().UTC()
	for i := range models {
		res, err := s.sdb.NewUpdate((*a2aDeliveryModel)(nil)).
			...
			Exec(ctx)
		if err != nil {
			return n, fmt.Errorf("cortex/sqlite: reclaim a2a delivery: %w", err)
		}
		affected, rowsErr := res.RowsAffected()
		if rowsErr != nil {
			return n, fmt.Errorf("cortex/sqlite: reclaim a2a delivery rows affected: %w", rowsErr)
		}
		n += int(affected)
	}
	return n, nil
...
func (s *Store) MarkDeliveryRead(ctx context.Context, deliveryID id.DeliveryID) error {
	...
	res, err := q.Exec(ctx)
	if err != nil {
		return fmt.Errorf("cortex/sqlite: mark a2a delivery read: %w", err)
	}
	n, rowsErr := res.RowsAffected()
	if rowsErr != nil {
		return fmt.Errorf("cortex/sqlite: mark a2a delivery read rows affected: %w", rowsErr)
	}
	if n == 0 {
...
func (s *Store) CreatePendingAsk(ctx context.Context, a *a2a.PendingAsk) error {
	...
	a.UpdatedAt = now
	a.Scope = scope
	if _, err := s.sdb.NewInsert(a2aPendingAskToModel(a)).Exec(ctx); err != nil {
		if isUniqueViolation(err) {
			return fmt.Errorf("cortex/sqlite: create a2a pending ask: %w", cortex.ErrAlreadyExists)
		}
		return fmt.Errorf("cortex/sqlite: create a2a pending ask: %w", err)
	}
	return nil
}
...
func (s *Store) ClaimPendingAsk(ctx context.Context, replyWith string) (*a2a.PendingAsk, error) {
	...
	now := time.Now().UTC()
	q := s.sdb.NewUpdate((*a2aPendingAskModel)(nil)).
		Set("claimed_at = ?", now).
		Set("updated_at = ?", now).
	...
	res, err := q.Exec(ctx)
	if err != nil {
		return nil, fmt.Errorf("cortex/sqlite: claim a2a pending ask: %w", err)
	}
	n, rowsErr := res.RowsAffected()
	if rowsErr != nil {
		return nil, fmt.Errorf("cortex/sqlite: claim a2a pending ask rows affected: %w", rowsErr)
	}
	if n == 0 {
...
func (s *Store) ListExpiredAsks(ctx context.Context, now time.Time, limit int) ([]*a2a.PendingAsk, error) {
	...
	if err := q.Scan(ctx); err != nil {
		return nil, fmt.Errorf("cortex/sqlite: list expired a2a asks: %w", err)
	}
...
func (s *Store) ListPendingAsksByConversation(ctx context.Context, convID id.ConversationID) ([]*a2a.PendingAsk, error) {
	...
	if err := q.Scan(ctx); err != nil {
		return nil, fmt.Errorf("cortex/sqlite: list a2a asks by conversation: %w", err)
	}
	return a2aPendingAsksFromModels(models)
}
...
func (s *Store) getDelivery(ctx context.Context, scope cortex.Scope, deliveryID id.DeliveryID) (*a2a.Delivery, error) {
	...
	q := s.sdb.NewSelect(m).Where("id = ?", deliveryID.String())
	for _, p := range scopePredicates(scope, false) {
		q = q.Where(p.Column+" = ?", p.Value)
	}
	if err := q.Scan(ctx); err != nil {
		...
		return nil, fmt.Errorf("cortex/sqlite: get a2a delivery: %w", err)
	}
...
func (s *Store) getPendingAsk(ctx context.Context, scope cortex.Scope, replyWith string) (*a2a.PendingAsk, error) {
	...
	q := s.sdb.NewSelect(m).Where("reply_with = ?", replyWith)
	for _, p := range scopePredicates(scope, false) {
		q = q.Where(p.Column+" = ?", p.Value)
	}
	if err := q.Scan(ctx); err != nil {
		...
		return nil, fmt.Errorf("cortex/sqlite: get a2a pending ask: %w", err)
	}
