...
func (s *Store) CreateSession(ctx context.Context, sess *session.Session) error {
	...
	if err != nil {
		if isUniqueViolation(err) {
			return fmt.Errorf("cortex/sqlite: create session: %w", cortex.ErrAlreadyExists)
		}
		return fmt.Errorf("cortex/sqlite: create session: %w", err)
	}
	return nil
}
...
func (s *Store) GetSession(ctx context.Context, sessionID id.SessionID) (*session.Session, error) {
	...
	m := new(sessionModel)
	q := s.sdb.NewSelect(m).Where("id = ?", sessionID.String())
	...
	if err != nil {
		...
		return nil, fmt.Errorf("cortex/sqlite: get session: %w", err)
	}
...
func (s *Store) UpdateSession(ctx context.Context, sess *session.Session) error {
	...
	m := sessionToModel(sess)
	q := s.sdb.NewUpdate(m).Column(mutableSessionColumns...).WherePK()
	for _, p := range scopePredicates(scope, false) {
		q = q.Where(p.Column+" = ?", p.Value)
	}
	res, err := q.Exec(ctx)
	if err != nil {
		return fmt.Errorf("cortex/sqlite: update session: %w", err)
	}
	n, rowsErr := res.RowsAffected()
	if rowsErr != nil {
		return fmt.Errorf("cortex/sqlite: update session rows affected: %w", rowsErr)
	}
	if n == 0 {
...
func (s *Store) DeleteSession(ctx context.Context, sessionID id.SessionID) error {
	...
	res, err := q.Exec(ctx)
	if err != nil {
		return fmt.Errorf("cortex/sqlite: delete session: %w", err)
	}
	n, rowsErr := res.RowsAffected()
	if rowsErr != nil {
		return fmt.Errorf("cortex/sqlite: delete session rows affected: %w", rowsErr)
	}
	if n == 0 {
		return cortex.ErrSessionNotFound
	}
...
func (s *Store) ListSessions(ctx context.Context, filter *session.ListFilter) ([]*session.Session, error) {
	...
	if err := q.Scan(ctx); err != nil {
		return nil, fmt.Errorf("cortex/sqlite: list sessions: %w", err)
	}
...
func (s *Store) CountSessions(ctx context.Context, filter *session.ListFilter) (int64, error) {
	...
	q := s.sdb.NewSelect((*sessionModel)(nil))
	for _, p := range scopePredicates(scope, filter.Exact) {
		q = q.Where(p.Column+" = ?", p.Value)
	}
	if !filter.AgentID.IsNil() {
	...
	if err != nil {
		return 0, fmt.Errorf("cortex/sqlite: count sessions: %w", err)
	}
	return count, nil
}
