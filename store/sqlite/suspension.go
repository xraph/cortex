...
func (s *Store) ClaimExpiredSuspension(ctx context.Context, runID id.AgentRunID, now time.Time) (*suspension.Suspension, error) {
	...
	tx, err := s.sdb.BeginTxQuery(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("cortex/sqlite: begin claim expired suspension: %w", err)
	}
	defer func() { _ = tx.Rollback() }() //nolint:errcheck // no-op after commit, unactionable otherwise
	...
	res, err := q.Exec(ctx)
	if err != nil {
		return nil, fmt.Errorf("cortex/sqlite: claim expired suspension: %w", err)
	}
	n, rowsErr := res.RowsAffected()
	if rowsErr != nil {
		return nil, fmt.Errorf("cortex/sqlite: claim expired suspension rows affected: %w", rowsErr)
	}
	if n == 0 {
	...
	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("cortex/sqlite: commit claim expired suspension: %w", err)
	}
	return suspensionFromModel(m)
}
