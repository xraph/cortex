...
VALUES ($1, $2, 'Default', $3, $4, TRUE, $5::jsonb, $6, $7, $8, '{}', $9, $10, $10)
...
// Metadata at all, so this only ever touches rows this migration itself
// wrote.
func unbackfillDefaultSessions(ctx context.Context, exec migrate.Executor) error {
	rows, err := exec.Query(ctx, `SELECT id FROM cortex_sessions WHERE metadata = $1::jsonb`, backfillSessionMarker)
	if err != nil {
