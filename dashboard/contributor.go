...
func (c *Contributor) renderMemory(ctx context.Context, s store.Store, params contributor.Params) (templ.Component, error) {
	...
	if agentIDStr != "" {
		...
		if parseErr == nil {
			sessionID := fetchDefaultSessionID(ctx, s, agID)
			messages, _ = s.LoadConversation(ctx, agID, sessionID, 100) //nolint:errcheck // best-effort UI data
		}
	}
...
func (c *Contributor) renderChat(ctx context.Context, s store.Store, params contributor.Params) (templ.Component, error) {
	...
	if selectedAgent != "" {
		...
		if err == nil {
			sessionID := fetchDefaultSessionID(ctx, s, ag.ID)
			messages, _ = s.LoadConversation(ctx, ag.ID, sessionID, 100) //nolint:errcheck // best-effort UI data
		}
	}
