...
**Scoped by design.** `cortex.WithScope` and `cortex.WithApp` inject context that is enforced at every layer. Cortex never interprets what a scope level means — the host defines its own hierarchy (workspace, org, tenant, whatever it needs) and Cortex just matches on it.
