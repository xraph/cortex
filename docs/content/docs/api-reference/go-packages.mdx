...
type Config struct {
    ...
    Sections []prompt.Section         // addressable prompt, wins over SystemPrompt
...
`Sections` and `SystemPrompt` have a stated precedence: a non-empty `Sections`
derives `SystemPrompt` and overwrites it, and an empty `Sections` with a
non-empty `SystemPrompt` lowers into one section with id `role`. See
[Prompt Composition](/docs/concepts/prompt-composition).
...
Prompt sections, the overlay that patches them, and the overlay store.

```go
type Section struct {
    ID, Body string
    Source   Source   // host, persona, skill, trait, knowledge
    Title    string   // prefixed to Body when non-empty
    Order    int      // low first, ties broken by ID
    Locked   bool     // refuses replace, still accepts append
}

type Patch struct {
    ID, Body string
    Mode     PatchMode // replace (default), append, and nothing else
}

func ApplyOverlay(sections []Section, patches []Patch) ([]Section, []Patch)
func Assemble(sections []Section) string

const OrderRole, OrderPersona, OrderSkill, OrderKnowledge, OrderTrait = 1000, 2000, 3000, 4000, 5000
...
type Overlay struct {
    ...
    Scope   cortex.Scope
    Patches []Patch
    ToolsAdded, ToolsRemoved []string
    Model       string
    Temperature *float64
    MaxTokens   *int
}

type Store interface {  // 7 methods
    CreateOverlay, GetOverlay, GetOverlayForAgent, GetOverlayForAgentAt,
    UpdateOverlay, DeleteOverlay, ListOverlays
}
```

`ApplyOverlay` returns the patches it declined as its second value. A `Locked`
section takes an append and nothing else, so a replace is declined and so is any
`Mode` outside the two constants. See
[Prompt Composition](/docs/concepts/prompt-composition).
...
| `prompt` | Domain | Prompt sections and scope overlays |
...
| `orchestration` | Execution | Multi-agent strategies, blackboard, Config/Run |
