...
type Patch struct {
    ID, Body string
    Mode     PatchMode // replace (default) or append
}
...
`ApplyOverlay` returns the patches it declined as its second value. A replace
against a `Locked` section is declined; an append is not. See
[Prompt Composition](/docs/concepts/prompt-composition).
