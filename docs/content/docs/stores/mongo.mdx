...
The MongoDB store (`store/mongo`) provides a document-oriented backend using the [Grove ORM](https://github.com/xraph/grove) with `mongodriver`. It stores Cortex entities in 13 MongoDB collections with compound indexes for efficient queries scoped by `cortex.Scope`.
...
| `cortex_orchestration_configs` | Orchestration configs |
| `cortex_orchestration_runs` | Orchestration runs |

## Composite store interface

The MongoDB store implements all 11 sub-interfaces:
