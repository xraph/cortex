...
The MongoDB store (`store/mongo`) provides a document-oriented backend using the [Grove ORM](https://github.com/xraph/grove) with `mongodriver`. It stores Cortex entities in 12 MongoDB collections with compound indexes for efficient queries scoped by `cortex.Scope`.
...
The MongoDB store implements all 10 sub-interfaces:
