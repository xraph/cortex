...
`cortex_runs`, `cortex_memories` and `cortex_checkpoints` also still carry a
`tenant_id` column, and `cortex_agents` still carries `app_id` — both
survive as read-only holdovers from before `Scope` (see
[Multi-tenancy](/docs/concepts/multi-tenancy) for why they aren't dropped
yet). Nothing reads or writes `tenant_id` anymore; `scope_l0`/`scope_l1`/
`scope_l2` plus the `scope_canon` string are what every scoped query
actually filters on. `cortex_orchestration_runs` does not get scope
columns — orchestration is out of scope for the current phase.
