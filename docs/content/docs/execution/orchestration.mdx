...
curl -X POST /cortex/v1/orchestrations -H 'Content-Type: application/json' -d '{
  "name": "should-we-ship",
  "strategy": "debate",
  "participants": [
...
# Run it
curl -X POST /cortex/v1/orchestrations/should-we-ship/run \
...
Run history is available at `GET /cortex/v1/orchestration-runs` and
