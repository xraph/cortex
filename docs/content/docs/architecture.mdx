...
│  3. Assemble prompt sections, then apply scope overlays           │
...
│  OnRunStarted         │  55 REST endpoints:                       │
│  OnRunCompleted       │  - Agents (8, incl. clone)                │
│  OnRunFailed          │  - Personas (6, incl. clone)              │
│  OnToolCalled         │  - Skills, Traits, Behaviors (5 each)     │
│  OnOrchestration*     │  - Orchestrations (8)                     │
│  OnAgentHandoff       │  - Runs (3), Checkpoints (2),             │
│  (16 total hooks)     │    Memory (2), Tools (2)                  │
├──────────────────────┴───────────────────────────────────────────┤
│                  orchestration (multi-agent)                      │
│  sequential │ parallel │ router │ hierarchical │ debate           │
│  Blackboard (shared state) · Roster (awareness) · Handoffs        │
├──────────────────────┬───────────────────────────────────────────┤
│                        store.Store                                │
│  (composite: agent + skill + trait + behavior + persona +         │
│   run + memory + checkpoint + orchestration.ConfigStore +         │
│   orchestration.RunStore + Migrate/Ping/Close)                    │
├──────────────────────────────────────────────────────────────────┤
│              store/sqlite · store/postgres · store/mongo          │
│  (grove ORM backends with programmatic migrations)                │
└──────────────────────────────────────────────────────────────────┘
```
...
3. **Assemble system prompt** from `prompt.Section` values: one for the agent's own instructions, one for the persona's `Identity`, one per skill `SystemPromptFragment`, one per retrieved knowledge source, and one per trait influence targeting `prompt_injection`. Then walk the run scope's ancestry, apply the `prompt.Overlay` found at each rung broadest first, and assemble the result in order. See [Prompt Composition](/docs/concepts/prompt-composition).
...
`cortex.WithScope(ctx, scope)` injects a scope into the context. Cortex never interprets what a scope level means: the host declares its own ordered hierarchy (workspace, org, tenant, app, whatever it needs) as a `cortex.Scope`, and Cortex only matches on it. This is extracted at every layer:

- **Store** — every scope-guarded query filters by the caller's scope, and rejects a zero scope with `cortex.ErrNoScope` instead of matching everything
- **Engine** — scope is read from the context and applied before any store operation
- **API** — the host's own middleware is responsible for calling `cortex.WithScope` on the request context before it reaches Cortex; the REST layer does not manufacture one

Cross-scope access is structurally impossible: even if a caller passes an agent ID from outside their scope, the store layer returns `ErrAgentNotFound`.
...
| `orchestration` | `.../orchestration` | Multi-agent strategies, Blackboard, Config/Run entities |
| `store` | `.../store` | Composite store interface |
| `store/sqlite` | `.../store/sqlite` | SQLite backend (grove ORM) |
| `store/postgres` | `.../store/postgres` | PostgreSQL backend (grove ORM) |
| `store/mongo` | `.../store/mongo` | MongoDB backend (grove ORM) |
...
| `api` | `.../api` | Forge-native HTTP handlers (46 routes) |
