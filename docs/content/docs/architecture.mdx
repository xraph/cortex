...
`cortex.WithScope(ctx, scope)` and `cortex.WithApp(ctx, id)` inject identifiers into the context. Cortex never interprets what a scope level means — the host declares its own ordered hierarchy (workspace, org, tenant, whatever it needs) as a `cortex.Scope`, and Cortex only matches on it. These are extracted at every layer:
