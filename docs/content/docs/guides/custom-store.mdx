...
1. **Scope isolation** — Every query must filter by the caller's scope and reject a zero scope with `cortex.ErrNoScope`. Use `cortex.ScopeFromContext(ctx)`; `agent.Store` methods also take `cortex.AppFromContext(ctx)`.
