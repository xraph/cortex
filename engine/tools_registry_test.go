...
func TestWithTool_DispatchedInExecuteTool(t *testing.T) {
	...
	got, _, _ := e.executeTool(context.Background(), cortex.Subject{}, llm.ToolCall{Name: "echo", Arguments: `{"x":1}`})
	if got != `echoed:{"x":1}` {
...
func TestExecuteTool_UnknownStillErrors(t *testing.T) {
	...
	got, _, _ := e.executeTool(context.Background(), cortex.Subject{}, llm.ToolCall{Name: "nope"})
